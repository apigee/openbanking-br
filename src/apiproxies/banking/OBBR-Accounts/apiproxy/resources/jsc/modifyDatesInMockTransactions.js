 ...
 */
 ...
 function modifyDateIfWithinRange(aDateStr) {
    ...
    var newDate = aDate;
    // Break date into year, month day, using "-" to split it
    var aDateStrDateArray = aDateStr.split("-");
    ...
    if (aDate != newDate) {
        ...
        aDateStr = newDate.getFullYear() +  "-" + padToTwoPlaces(newDate.getMonth()+1) + "-" + padToTwoPlaces(newDate.getDate());
 ...
 if (txList && txList.length > 0) {
    for (var i = 0; i < txList.length; i++) {
        curTx = txList[i];
        print("Working on tx # " + i + " - transactionDate = " + curTx.transactionDate);
        curTx.transactionDate = modifyDateIfWithinRange(curTx.transactionDate);
        print("Now transactionDate = " + curTx.transactionDate);
        txList[i] = curTx;
